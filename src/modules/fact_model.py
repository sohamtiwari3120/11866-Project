...
class FACTModel(nn.Module):
  ...
  def __init__(self, config, mask_index=-1, quant_factor=None, use_text_transcriptions=True, use_concat_attention = True):
    ...
    self.listener_past_transformer = Transformer(
        in_size=self.config['fact_model']['listener_past_transformer_config']\
                           ['hidden_size'],
        hidden_size=self.config['fact_model']['listener_past_transformer_config']\
                               ['hidden_size'],
        num_hidden_layers=self.config['fact_model']['listener_past_transformer_config']\
                                     ['num_hidden_layers'],
        num_attention_heads=self.config['fact_model']['listener_past_transformer_config']\
                                       ['num_attention_heads'],
        intermediate_size=self.config['fact_model']['listener_past_transformer_config']\
                                     ['intermediate_size'])
    self.listener_past_pos_embedding = PositionEmbedding(
        self.config['fact_model']["listener_past_transformer_config"]["sequence_length"],
        self.config['fact_model']['listener_past_transformer_config']['hidden_size'])
    self.listener_past_tok_embedding = nn.Embedding(
        self.config['fact_model']['listener_past_transformer_config']['in_dim'],
        self.config['fact_model']['listener_past_transformer_config']['hidden_size'])
    ...
    dim = self.config['fact_model']['speaker_full_transformer_config']['hidden_size']
    ...
    self.cross_modal_layer = CrossModalLayer(self.config['fact_model']['cross_modal_model'])
  ...
  def forward(self, inputs, max_mask, mask_index):
    ...
    F = self.config['fact_model']["listener_past_transformer_config"]["hidden_size"]
